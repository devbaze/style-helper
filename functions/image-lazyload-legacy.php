...
if ( ! function_exists( 'get_image_lazyload_div' ) ) {
  function get_image_lazyload_div( $image_id = 0, $sizes = [], $fallback = false ) {
    // Get image
    $image_urls = helper_get_image_lazyload_sizes( $image_id, $sizes );
    ...
    $styles = '';
    $styles_array = apply_filters( 'image_lazyload_div_styles', [], $image_id ); // backwards comp
    $styles_array = apply_filters( 'helper_image_lazyload_div_styles', $styles_array, $image_id );
    ...
    <?php if ( apply_filters( 'helper_image_lazyload_enable_noscript_fallback', false ) ) : ?>
      <noscript>
        <div aria-hidden="true"
          class="background-image full-image"
          style="background-image: url('<?php echo esc_url( $image_urls['big'] ); ?>'); <?php echo esc_attr( $styles ); ?>">
...
if ( ! function_exists( 'get_image_lazyload_div_fallback' ) ) {
  function get_image_lazyload_div_fallback( $fallback = false ) {
    if ( empty( $fallback ) ) {
      if ( apply_filters( 'helper_image_lazyload_fallback_from_theme_settings', true ) && defined( 'THEME_SETTINGS' ) ) {
...
if ( ! function_exists( 'get_image_lazyload_tag' ) ) {
  function get_image_lazyload_tag( $image_id = 0, $sizes = [], $fallback = false ) {
    ...
    // Get dimensions
    $dimensions = helper_get_image_lazyload_dimensions( $image_id, $sizes );
